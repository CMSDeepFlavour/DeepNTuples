...
if options.isData:
    if options.deepNtuplizer:
        process.p = cms.Path(process.TriggerSel + process.FinalSel + process.QGTagger + process.deepntuplizer)
    else:
        process.p = cms.Path(process.TriggerSel + process.FinalSel + process.QGTagger)
else:
    if options.deepNtuplizer:
        process.p = cms.Path(process.TriggerSel + process.FinalSel + process.QGTagger + process.genJetSequence * process.deepntuplizer)
    else:
        process.p = cms.Path(process.TriggerSel + process.FinalSel + process.QGTagger + process.genJetSequence)
