    ...
    for root1, dirs, files in os.walk(root+x):
        for file in files:
            if file.endswith(".root"):
                index = str(nr)
                call(["weightDistribution", os.path.join(root1,file), outputdir+x+outfile+index+outend, "DATA"]) 
                nr += 1
                if nr%20 == 0:
                    print(nr)
...
for x in mcdir:
    for file in os.listdir(root+x):
        if file.endswith(".root"):
            index = str(nr)
            call(["weightDistribution", os.path.join(root+x,file), outputdir+x+outfile+index+outend, "MC"])
            nr += 1
            if nr%20 == 0:
                print(nr)
