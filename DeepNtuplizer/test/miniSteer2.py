...
root = '/afs/cern.ch/work/e/ebols/public/TestSample'
...
for root, dirs, files in os.walk(root):
    for file in files:
        if file.startswith("data"):
            index = str(blah)
            call(["NtupleConverter", os.path.join(root,file), outputdir+outfile+index+outend, "DATA"])
            blah += 1
            print(blah)
        if file.startswith("MC"):
            index = str(blah)
            call(["NtupleConverter", os.path.join(root,file), outputdir+outfile+index+outend, "MC"])
            blah += 1
            print(blah)
