...
class scaleFactor():
    """
     ...
     hist is the 2d histogram with the scale factor with x and y
    """
...
#muonlabel = ("GoodMuon")
    ...
    triggerhandle = Handle("edm::TriggerResults")
    triggerlabel = ("TriggerResults::HLT")
    ...
    def __init__(self, name, isData=False, color=1):
        ...
        if self.isData==False:
            ...
            self.h_ll_eta.SetYTitle("Events")
            self.h_tl_pt.SetYTitle("Events/GeV")
            self.h_tl_eta.SetYTitle("Events")
    ...
    def fillHists(self, infile, islist=True, useLHEWeights=False, isData=False, eventIDs=[]):
        ...
        while ifile != '':
            ...
            for event in events:
                ...
                lumi_BCDEF = 5.404+2.396+4.243+4.054+3.105
                lumi_G = 7.544
                lumi_H = 8.453
                ...
                if isData == False:
                    ...
                    weight_BCDEF *= process.sf_muon_id_BCDEF.getScalefactor(abs(leadingMuonEta), leadingMuonPt)
                    weight_BCDEF *= process.sf_muon_iso_BCDEF.getScalefactor(abs(leadingMuonEta), leadingMuonPt)
                    weight_GH *= process.sf_muon_id_GH.getScalefactor(abs(leadingMuonEta), leadingMuonPt)
                    weight_GH *= process.sf_muon_iso_GH.getScalefactor(abs(leadingMuonEta), leadingMuonPt)
                    weight *= process.sf_el_id.getScalefactor(abs(leadingElSuEta), leadingElPt)

                    ### For 2016H there was used another emu trigger (DZ version) then for B to G,
                    #       compute weights for events that pass the "2016H trigger" and for those who pass the "B to G trigger"
                    trigger_BCDEFG = 0
                    trigger_H = 0
                    event.getByLabel(process.triggerlabel, process.triggerhandle)
                    trigger = process.triggerhandle.product()
                    for i in range(trigger.size()):
                        #print(i, events.object().triggerNames(trigger).triggerName(i), trigger.accept(i))
                        if events.object().triggerNames(trigger).triggerName(i)[0:49] == "HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL_v" or \
                                        events.object().triggerNames(trigger).triggerName(i)[0:48] == "HLT_Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_v":

                            if trigger.accept(i):
                                trigger_BCDEFG = 1

                        if events.object().triggerNames(trigger).triggerName(i)[0:52] == "HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL_DZ_v" or \
                                        events.object().triggerNames(trigger).triggerName(i)[0:51] == "HLT_Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_DZ_v":
                            if trigger.accept(i):
                                trigger_H = 1

                    weight = weight * (weight_BCDEF * lumi_BCDEF * trigger_BCDEFG
                                       + weight_GH * lumi_G * trigger_BCDEFG
                                       + weight_GH * lumi_H * trigger_H)
    ...
    def load_hists(self):
        ...
        if(os.path.isfile(self.name + ".root")):
            ...
            print("no file for this process found")
...
data.setMinMax(0,8000,0,40000,0,17000,0,40000,0,140000)

title='plots_180514_full2016'
directory = os.path.dirname('./'+title+'/')
# make a canvas, draw, and save it
if os.path.exists(directory):
    print("ERROR: output directory exists, please choose another name or delite the old one")
    import sys
    sys.exit()
else:
